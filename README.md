...
═══ УСТАНОВКА ═══
  1) Установить прокси            (домен, секрет, AD_TAG)
  2) Настроить безопасность VPS   (ufw, fail2ban, sysctl)

═══ УПРАВЛЕНИЕ ═══
  3) Статус контейнеров
  4) Логи alexbers                (live, Ctrl+C — выход)
  5) Логи Caddy                   (live, Ctrl+C — выход)
  6) Перезапустить прокси
  7) Остановить всё
  8) Запустить всё
  9) Показать ссылку для пользователей

═══ ОБСЛУЖИВАНИЕ ═══
  10) Обновить скрипт из git
  11) Удалить прокси
